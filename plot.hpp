...
public :
	...
	plot(TString sample, TTree *tree=0);
	virtual ~plot();
...
#ifdef plot_cxx
plot::plot(TString sample, TTree *tree) : fChain(0) 
{
...
TH1 * plot::MakePlot(TString name, int nbins, float xlow, float xhigh) {
	TH1 *tmp = new TH1F(name, name, nbins, xlow, xhigh);
	tmp->Sumw2();
	return tmp;
}
TH1 * plot::MakePlot(TString name, int nbins, double *xbins) {
	TH1 *tmp = new TH1F(name, name, nbins, xbins);
	tmp->Sumw2();
	return tmp;
}
TH2 * plot::MakePlot(TString name, int nbinsx, float xlow, float xhigh, int nbinsy, float ylow, float yhigh) {
	TH2 *tmp = new TH2F(name, name, nbinsx, xlow, xhigh, nbinsy, ylow, yhigh);
	tmp->Sumw2();
	return tmp;
}
TH2 * plot::MakePlot(TString name, int nbinsx, double *xbins, int nbinsy, float ylow, float yhigh) {
...
std::pair<int, int> plot::Label(float eta, float phi, float mv1) {
...
void plot::Relabel(TH1 *&plot, bool isb) {
...
float plot::hadron_weight(int hlabel) {
...
double plot::b_pT_reweight(Double_t pT) {
	double par[3];
	par[0] = 0.874857;
	par[1] = 0.00374837;
	par[2] = -6.54698e-06;
...
double plot::c_pT_reweight(Double_t pT) {
	double par[3];
	par[0] = 1.05691;
	par[1] = 0.000752842;
	par[2] = 2.46003e-06;
...
double plot::weight(TH1 * plot, double val) {
...
double plot::weight(TH2 * plot, double valx, double valy) {
	if(m_sample != "187007") return 1.0;	
...
TH1* plot::Ratio(TH1 * plot1, TH1*plot2) {
